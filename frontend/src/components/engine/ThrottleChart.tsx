...
import type { ThrottleComparisonResponse } from "@/lib/api";

interface Props {
  data: ThrottleComparisonResponse | null;
  cursorIndex: number | null;
}

const ThrottleChart = ({ data, cursorIndex }: Props) => {
  const chartData = useMemo(() => {
    if (!data) return [];
    const { distanceM, lapThrottlePct, referenceThrottlePct, deltaThrottlePct } = data.series;
    return distanceM.map((d, i) => ({
      distance: Math.round(d),
      lap: lapThrottlePct[i],
      ref: referenceThrottlePct[i],
      delta: deltaThrottlePct[i],
    }));
  }, [data]);
  ...
  if (!data) {
    return (
      <div className="border border-border rounded-lg bg-card flex items-center justify-center">
        <span className="text-muted-foreground text-xs">Loading throttle data…</span>
      </div>
    );
  }
        ...
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={chartData} margin={{ top: 8, right: 12, bottom: 4, left: 0 }}>
            ...
            {data.highlights.map((h, i) => (
              <ReferenceArea
                key={i}
                x1={Math.round(h.startDistanceM)}
                x2={Math.round(h.endDistanceM)}
                fill="hsl(0, 85%, 50%)"
                fillOpacity={0.08}
                strokeOpacity={0}
              />
            ...
            <YAxis
              ...
              axisLine={false}
              width={32}
              label={{ value: "%", angle: -90, position: "insideLeft", fontSize: 9, fill: "hsl(220, 10%, 45%)" }}
            />
            <Tooltip
              contentStyle={{
              ...
              formatter={(value: number, name: string) => [
                `${value.toFixed(1)}%`,
                name === "lap" ? "Your Lap" : name === "ref" ? "Reference" : "Delta",
              ]}
            />
